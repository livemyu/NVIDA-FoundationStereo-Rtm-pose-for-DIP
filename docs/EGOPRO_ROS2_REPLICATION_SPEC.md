# EgoPro 具身智能第一人称多模态数据集 ROS 2 话题发布标准与复刻规范

本文档记录并规范了 **EgoPro（第一人称具身智能多模态数据标准）** 的全套 ROS 2 话题体系、Protobuf 数据模式（Schema）、消息字段结构及时间戳对齐标准，作为后续在 Jetson Orin / 双目视觉硬件上全流程复刻、采集并录制该标准数据集的权威技术规范。
